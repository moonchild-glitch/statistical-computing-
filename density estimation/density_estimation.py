...
# Generate Old Faithful-like data (bimodal distribution)
# Real Old Faithful data shows eruption durations with two modes
np.random.seed(42)
# Create bimodal data: mixture of two normal distributions
faithful_eruptions = np.concatenate([
    np.random.normal(2.0, 0.5, 130),  # Short eruptions
    np.random.normal(4.5, 0.5, 140)   # Long eruptions
])
...
# Compute histogram
counts, edges = np.histogram(faithful_eruptions, bins=my_breaks)
mids = (edges[:-1] + edges[1:]) / 2
density = counts / (len(faithful_eruptions) * h)

# Plot the histogram as a step function showing density
plt.figure(figsize=(10, 6))
plt.step(mids, density, where='mid', linewidth=2)
plt.xlabel('Eruption length')
plt.ylabel('Density')
plt.title('Histogram of Eruption Lengths')
plt.grid(True, alpha=0.3)
plt.savefig('plots/01_histogram_faithful.pdf')
plt.close()
...
x_grid = np.arange(0, 6.2, 0.2)
m = len(x_grid)
n = len(faithful_eruptions)
h = 0.5  # Bandwidth
...
plt.figure(figsize=(10, 6))
plt.step(x_grid, fhat, where='mid', linewidth=2)
plt.xlabel('Eruption length')
plt.ylabel('Density Estimate')
plt.title('Naive Density Estimator')
plt.grid(True, alpha=0.3)
plt.savefig('plots/02_naive_density_estimator.pdf')
plt.close()
...
def my_w(x, kernel_type="gaussian"):
    """
...
plt.figure(figsize=(10, 6))
plt.plot(x_grid, fhat, linewidth=2)
plt.xlabel('Eruption length')
plt.ylabel('Density Estimate')
plt.title('Kernel Density Estimator (Gaussian)')
plt.grid(True, alpha=0.3)
plt.savefig('plots/03_gaussian_kernel_density.pdf')
plt.close()
...
# - scipy.stats.gaussian_kde uses this by default (Scott's rule)
#
...
# 2. Draw from N(xᵢ, h²)
#    Use: np.random.normal(x[i], h)
#
# For q draws:
#    np.random.normal(np.random.choice(x, q), h)
#
# Interpretation:
...
# - For uniform kernel: np.random.uniform with appropriate range
...
# scipy.stats.gaussian_kde is the most common built-in function for kernel 
# density estimation in Python
...
# Key features:
# - Automatic bandwidth selection using Scott's rule by default
# - Can specify custom bandwidth with bw_method parameter
#   * 'scott' (default): 1.059 * σ * n^(-1/5)
#   * 'silverman': 1.06 * σ * n^(-1/5)
#   * float: custom bandwidth factor
#   * callable: custom bandwidth function
# - Only supports Gaussian kernel
# - Fast and efficient for large datasets

# Example usage:
# kde = gaussian_kde(faithful_eruptions)
# x_grid = np.linspace(0, 8, 200)
# density = kde(x_grid)
# plt.plot(x_grid, density)

# Other Python packages for density estimation:
# - sklearn.neighbors.KernelDensity: more kernel options, cross-validation
# - statsmodels.nonparametric.kde: additional bandwidth selection methods
# - KDEpy: fast kernel density estimation with various kernels
# - seaborn: high-level plotting interface with built-in KDE
...
# Can use numpy.histogram2d function to bin a bivariate data set
def bin2d(x, bins1='sturges', bins2='sturges'):
    """
    ...
    x : array-like, shape (n, 2)
        Two-column array of data points
    bins1, bins2 : int, str, or array-like
        Binning strategy for each dimension
    ...
    """
    # Determine bin edges for each dimension
    if bins1 == 'sturges':
        n = len(x)
        nbins1 = int(np.ceil(np.log2(n) + 1))
        bins1 = nbins1
    if bins2 == 'sturges':
        n = len(x)
        nbins2 = int(np.ceil(np.log2(n) + 1))
        bins2 = nbins2
    
    # Compute 2D histogram
    freq, edges1, edges2 = np.histogram2d(x[:, 0], x[:, 1], bins=[bins1, bins2])
    
    # Calculate midpoints
    mids1 = (edges1[:-1] + edges1[1:]) / 2
...
#
# Usage:
# data_2d = np.column_stack([x1, x2])  # Two-column matrix
# bins = bin2d(data_2d)
# plt.imshow(bins['freq'].T, origin='lower')  # Heatmap
# plt.contour(bins['mids1'], bins['mids2'], bins['freq'].T)  # Contour plot
...
# for the first species (setosa, rows 0:50)
iris_setosa = iris_data[0:50, 0:2]
fit1 = bin2d(iris_setosa)

# After binning the data, create 3D perspective plot
fig = plt.figure(figsize=(12, 8))
ax = fig.add_subplot(111, projection='3d')

# Create meshgrid for plotting
X, Y = np.meshgrid(fit1['mids1'], fit1['mids2'])

# Plot surface
surf = ax.plot_surface(X, Y, fit1['freq'].T, cmap=cm.viridis,
                       linewidth=0, antialiased=True, alpha=0.8)
ax.set_xlabel('Sepal Length')
ax.set_ylabel('Sepal Width')
ax.set_zlabel('Frequency')
ax.set_title('Bivariate Density Histogram - Iris Setosa')
ax.view_init(elev=30, azim=45)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.savefig('plots/04_iris_bivariate_3d.pdf')
plt.close()

# Alternative visualizations:
# Heatmap
plt.figure(figsize=(10, 8))
plt.imshow(fit1['freq'].T, origin='lower', cmap='viridis', aspect='auto',
           extent=[fit1['edges1'][0], fit1['edges1'][-1],
                   fit1['edges2'][0], fit1['edges2'][-1]])
plt.colorbar(label='Frequency')
plt.xlabel('Sepal Length')
plt.ylabel('Sepal Width')
plt.title('Bivariate Density - Heatmap')
plt.savefig('plots/05_iris_heatmap.pdf')
plt.close()

# Contour plot
plt.figure(figsize=(10, 8))
X, Y = np.meshgrid(fit1['mids1'], fit1['mids2'])
plt.contour(X, Y, fit1['freq'].T, levels=10, cmap='viridis')
plt.colorbar(label='Frequency')
plt.xlabel('Sepal Length')
plt.ylabel('Sepal Width')
plt.title('Bivariate Density - Contour Plot')
plt.savefig('plots/06_iris_contour.pdf')
plt.close()
...
n = 2000
p = np.array([0.2, 0.3, 0.5])  # Mixture probabilities
mu = np.array([[0, 0], [1, 3], [4, -1]])  # Means: rows are components
Sigma = np.eye(2)  # Identity covariance matrix

# Sample component labels according to mixture probabilities
i = np.random.choice([0, 1, 2], size=n, replace=True, p=p)
k = np.bincount(i, minlength=3)  # Count observations from each component

# Generate samples from each component
x1 = np.random.multivariate_normal(mu[0], Sigma, size=k[0])
x2 = np.random.multivariate_normal(mu[1], Sigma, size=k[1])
x3 = np.random.multivariate_normal(mu[2], Sigma, size=k[2])
...
# Using scipy's gaussian_kde with custom bandwidth
from scipy.stats import gaussian_kde
...
fhat = np.reshape(kde(positions), xx.shape)
...
ax1.contour(xx, yy, fhat, levels=10, cmap='viridis')
ax1.set_xlabel('x')
ax1.set_ylabel('y')
ax1.set_title('Bivariate KDE - Contour')

# 3D surface plot
ax2 = fig.add_subplot(122, projection='3d')
surf = ax2.plot_surface(xx, yy, fhat, cmap=cm.viridis,
                        linewidth=0, antialiased=True, alpha=0.8)
ax2.set_xlabel('x')
ax2.set_ylabel('y')
ax2.set_zlabel('Density')
ax2.set_title('Bivariate KDE - 3D Surface')
ax2.view_init(elev=30, azim=20)

plt.tight_layout()
plt.savefig('plots/07_bivariate_mixture_density.pdf')
plt.close()

print("\n=== All plots saved to plots/ directory ===")
print("Generated files:")
print("  01_histogram_faithful.pdf")
print("  02_naive_density_estimator.pdf")
print("  03_gaussian_kernel_density.pdf")
print("  04_iris_bivariate_3d.pdf")
print("  05_iris_heatmap.pdf")
print("  06_iris_contour.pdf")
print("  07_bivariate_mixture_density.pdf")
